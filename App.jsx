...
// ─── CONSTANTS ────────────────────────────────────────────────────────────────
var GROUPS = {
  A: ["Mexico","South Korea","South Africa","UEFA PO-D"],
  ...
  K: ["Portugal","Colombia","Uzbekistan","IC PO-1"],
  L: ["England","Croatia","Ghana","Panama"]
};

var GROUP_FIXTURES = {
  A:[["Mexico","South Africa"],["South Korea","UEFA PO-D"],["Mexico","South Korea"],["South Africa","UEFA PO-D"],["Mexico","UEFA PO-D"],["South Korea","South Africa"]],
  B:[["Canada","UEFA PO-A"],["Qatar","Switzerland"],["Canada","Qatar"],["Switzerland","UEFA PO-A"],["Canada","Switzerland"],["UEFA PO-A","Qatar"]],
  C:[["Brazil","Morocco"],["Haiti","Scotland"],["Brazil","Haiti"],["Scotland","Morocco"],["Brazil","Scotland"],["Morocco","Haiti"]],
  D:[["USA","Paraguay"],["Australia","UEFA PO-C"],["USA","Australia"],["UEFA PO-C","Paraguay"],["USA","UEFA PO-C"],["Paraguay","Australia"]],
  E:[["Germany","Curaçao"],["Côte d'Ivoire","Ecuador"],["Germany","Côte d'Ivoire"],["Ecuador","Curaçao"],["Germany","Ecuador"],["Curaçao","Côte d'Ivoire"]],
  F:[["Netherlands","Japan"],["UEFA PO-B","Tunisia"],["Netherlands","UEFA PO-B"],["Tunisia","Japan"],["Netherlands","Tunisia"],["Japan","UEFA PO-B"]],
  ...
  I:[["France","Senegal"],["IC PO-2","Norway"],["France","IC PO-2"],["Norway","Senegal"],["France","Norway"],["Senegal","IC PO-2"]],
  J:[["Argentina","Algeria"],["Austria","Jordan"],["Argentina","Austria"],["Jordan","Algeria"],["Argentina","Jordan"],["Algeria","Austria"]],
  K:[["Portugal","IC PO-1"],["Uzbekistan","Colombia"],["Portugal","Uzbekistan"],["Colombia","IC PO-1"],["Portugal","Colombia"],["IC PO-1","Uzbekistan"]],
...
var FLAG_CODES = {
  "Mexico":"mx","South Korea":"kr","South Africa":"za","UEFA PO-D":"eu",
  "Canada":"ca","Switzerland":"ch","Qatar":"qa","UEFA PO-A":"eu",
  "Brazil":"br","Morocco":"ma","Haiti":"ht","Scotland":"gb-sct",
  "USA":"us","Paraguay":"py","Australia":"au","UEFA PO-C":"eu",
  "Germany":"de","Curaçao":"cw","Côte d'Ivoire":"ci","Ecuador":"ec",
  "Netherlands":"nl","Japan":"jp","UEFA PO-B":"eu","Tunisia":"tn",
  "Belgium":"be","Egypt":"eg","Iran":"ir","New Zealand":"nz",
  "Spain":"es","Saudi Arabia":"sa","Cabo Verde":"cv","Uruguay":"uy",
  "France":"fr","Senegal":"sn","IC PO-2":"un","Norway":"no",
  "Argentina":"ar","Algeria":"dz","Austria":"at","Jordan":"jo",
  "Portugal":"pt","Colombia":"co","Uzbekistan":"uz","IC PO-1":"un",
...
function storeGet(k) {
  try {
    // Try Claude.ai window.storage first, fall back to localStorage
    ...
    // localStorage fallback (Vercel / local dev)
    var val = localStorage.getItem(k);
...
function storeSet(k, v) {
  try {
    ...
    // Always write to localStorage so data persists
    try { localStorage.setItem(k, json); } catch(e) {}
    // Also write to window.storage if available (Claude.ai)
    if (window.storage && typeof window.storage.set === "function") {
      return window.storage.set(k, json, true).catch(function() {});
    }
...
function resolveSlot(slot, gRes, best8) {
...
function calcScore(pred, results, penalty, isLate) {
  ...
  var keys = Object.keys(pSc);
  for (var ki = 0; ki < keys.length; ki++) {
    var ps = pSc[keys[ki]]; var rs = rSc[keys[ki]];
    if (rs && ps && String(ps.h) === String(rs.h) && String(ps.a) === String(rs.a)) pts += SCORE_BONUS;
  }
...
function getBreakdown(pred, results, penalty, isLate) {
  ...
  for (var gi2 = 0; gi2 < gkeys.length; gi2++) {
    ...
    for (var qi = 0; qi < qTeams.length; qi++) {
      var qt = qTeams[qi];
      if (!qt) continue;
      var qhit = qt === rQ.gRes[g2].winner || qt === rQ.gRes[g2].runnerUp;
      bd.groupQualifiers.push({team:qt, label:qt+" (Grp "+g2+" "+(qi===0?"1ro":"2do")+")", hit:qhit, pts:qhit?GQ_PTS:0});
    }
  }
  for (var b8i = 0; b8i < pQ.best8.length; b8i++) {
    var bt = pQ.best8[b8i];
    var bhit = rQ.best8.indexOf(bt) >= 0;
    bd.best3rd.push({team:bt, label:bt, hit:bhit, pts:bhit?GQ_PTS:0});
  }
...
function PredictView(props) {
  ...
  function buildR32Pairs() {
    ...
    for (var i = 0; i < R32_SLOTS.length; i++) {
      ...
      pairs.push({
        a: resolveSlot(slot.a, qualData.gRes, qualData.best8),
        b: resolveSlot(slot.b, qualData.gRes, qualData.best8),
        label: slot.label
      });
    }
    return pairs;
  ...
  function getPairs(advKey, pairDefs) {
    var adv = pred[advKey + "_adv"] || [];
    var result = [];
    for (var i = 0; i < pairDefs.length; i++) {
      result.push({a: adv[pairDefs[i][0]]||"TBD", b: adv[pairDefs[i][1]]||"TBD", label:""+(i+1)});
    }
    return result;
  }
  ...
  return (
    <div style={{...S.page, maxWidth:660}}>
      ...
      {tab === "r16"    && <BracketRound advKey="r16" pairs={getPairs("r32", R16_PAIRS)} pred={pred} setWinner={isLocked?noop:setWinner} locked={isLocked} hint="Completa la Ronda de 32 primero." />}
      {tab === "qf"     && <BracketRound advKey="qf"  pairs={getPairs("r16", QF_PAIRS)}  pred={pred} setWinner={isLocked?noop:setWinner} locked={isLocked} hint="Completa la Ronda de 16 primero." />}
      {tab === "sf"     && <BracketRound advKey="sf"  pairs={getPairs("qf",  SF_PAIRS)}  pred={pred} setWinner={isLocked?noop:setWinner} locked={isLocked} hint="Completa los cuartos primero." />}
...
function FinalTab(props) {
  ...
  return (
    <div>
      <p style={S.hint}>Elige tu campeón del Mundial.</p>
      <div style={S.card}>
                      ...
                      {items.map(function(item, idx) {
                        return (
                          <div key={idx} style={{display:"flex",alignItems:"center",gap:10,padding:"8px 10px",borderRadius:8,background:item.hit?"#EAF3DE":"var(--color-background-secondary)",border:"0.5px solid "+(item.hit?"#3B6D11":"var(--color-border-tertiary)")}}>
                            <span style={{fontSize:15,flexShrink:0}}>{item.hit ? "✅" : hasR ? "❌" : "⏳"}</span>
                            ...
                            <span style={{fontSize:12,fontWeight:500,color:item.hit?"#27500A":"var(--color-text-tertiary)",flexShrink:0}}>
                              {item.hit ? ("+" + item.pts + " pts") : hasR ? "0 pts" : "?"}
                            </span>
                          </div>
                        );
                      })}
  ...
  if (!props.adminUnlocked) {
    return (
      <div style={S.page}>
        ...
        <p style={{fontSize:12,color:"var(--color-text-tertiary)",marginTop:10}}>Por defecto: admin2026</p>
      </div>
    );
  }
          ...
          {approved.length > 0 && (
            <div>
              ...
              {approved.map(function(p) {
                return (
                  <div key={p.id} style={{display:"flex",alignItems:"center",gap:10,padding:"8px 0",borderBottom:"0.5px solid var(--color-border-tertiary)"}}>
      ...
      {tab === "knockout" && (
        <div>
          ...
          <div style={{display:"flex",flexDirection:"column",gap:8,marginBottom:20}}>
            {R32_SLOTS.map(function(slot, i) {
              var tA = resolveSlot(slot.a, aQ.gRes, aQ.best8);
              var tB = resolveSlot(slot.b, aQ.gRes, aQ.best8);
          ...
          {[
            {title:"Ronda de 16",pairs:R16_PAIRS,advKey:"r16",srcKey:"r32"},
            {title:"Cuartos de final",pairs:QF_PAIRS,advKey:"qf",srcKey:"r16"},
            {title:"Semifinales",pairs:SF_PAIRS,advKey:"sf",srcKey:"qf"}
          ].map(function(rd) {
              ...
              <div key={rd.title} style={{marginBottom:20}}>
                ...
                <div style={{display:"flex",flexDirection:"column",gap:8}}>
                  {rd.pairs.map(function(pair, i) {
                    ...
                    return (
                      <div key={i} style={{...S.card, display:"flex", alignItems:"center", gap:8}}>
                        <div style={{fontSize:11,color:"var(--color-text-tertiary)",minWidth:36,textAlign:"center",fontWeight:500}}>{i+1}</div>
